...
class Batchmaker:
    ...
    def next_batch(self):
        ...
        for i_example in range(self.batch_size):
            ...
            next_sample = self.input_data[pos]
            if (np.random.uniform(0,1) > 0.5):
                next_sample = np.fliplr(next_sample)
            if (np.random.uniform(0,1) > 0.5):
                next_sample = next_sample + (np.sign(np.random.uniform(-1,1))) * np.random.uniform(0,5)
                next_sample = np.clip(next_sample,0, 255)

            batch_input_values[i_example] = np.reshape(next_sample, [self.input_shape])
